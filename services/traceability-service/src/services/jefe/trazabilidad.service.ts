...
export class TrazabilidadService {
  ...
  async createTrazabilidad(data: any) {
    ...
    if (!ejecutiva) throw new HttpException('Ejecutiva no encontrada', HttpStatus.BAD_REQUEST);
    if (!empresa) throw new HttpException('Empresa proveedora no encontrada', HttpStatus.BAD_REQUEST);
    if (!cliente) throw new HttpException('Cliente final no encontrado', HttpStatus.BAD_REQUEST);
    if (!contacto) throw new HttpException('Persona de contacto no encontrada', HttpStatus.BAD_REQUEST);
  ...
  async getEtapa1(filters: {
    ...
    const query = this.trazabilidadRepository.createQueryBuilder('t')
      ...
      // ⚠️ REMOVER completamente esta línea de filtro:
    ...
    // Los demás filtros se mantienen igual...
    if (filters.ejecutivaId) {
      query.andWhere('t.id_ejecutiva = :ejecutivaId', { ejecutivaId: filters.ejecutivaId });
    }
    if (filters.empresaId) {
  ...
  async getEtapa2(filters: {
    ...
    const etapa2Data = data.map(t => ({
      ...
      probabilidad_cierre: t.probabilidad_cierre || 0, // ← Cambiar a snake_case
  ...
  async getNuevasReunionesAgendadas(meses: number = 6, ejecutivaId?: number) {
    ...
    const query = this.trazabilidadRepository.createQueryBuilder('t')
      ...
      .andWhere('t.fecha_reunion IS NOT NULL') // Solo reuniones con fecha
    ...
    // Si no hay datos, retornar array vacío en lugar de datos de prueba
    return [];
  }
  ...
  async getResumenSemanalEjecutivas() {
    // CAMBIAR: En lugar de filtrar por última semana, mostrar todos los datos
    const query = this.trazabilidadRepository.createQueryBuilder('t')
      .leftJoin('t.ejecutiva', 'ej')
      .select('ej.id_ejecutiva', 'id_ejecutiva')
      .addSelect('ej.nombre_completo', 'ejecutiva')
      ...
      .where('t.fecha_contacto IS NOT NULL') // Solo actividades con fecha
      .groupBy('ej.id_ejecutiva, ej.nombre_completo')
  ...
  async getEmbudoVentas(filters?: {
    ...
    const query = this.trazabilidadRepository.createQueryBuilder('t')
      ...
      .groupBy('t.etapa_oportunidad')
      .orderBy('COUNT(t.id_trazabilidad)', 'DESC'); // ← ORDENAR POR CANTIDAD DESC
    ...
    // Crear embudo con porcentajes correctos
    const embudo = data.map(d => {
      ...
      return {
        ...
        tasa_conversion: porcentajeDesdeInicio, // ← ESTE es el porcentaje que usa el frontend
        perdida: 0
      };
    });
    ...
    private acortarEtapa(etapa: string): string {
      const acortamientos: {[key: string]: string} = {
        'Prospección': 'Prosp.',
        'Calificación': 'Calif.',
  ...
  // MÉTODO CORREGIDO: getRankingEjecutivas
  ...
  // ============================================
// MÉTODOS PARA REPORTES - CORREGIDOS
// ============================================
  ...
  // ============================================
  // MÉTODO AUXILIAR PARA FORMATEAR FECHAS EN CSV
  // ============================================

  private formatDateForCSV(dateValue: any): string {
    if (!dateValue) return '';
